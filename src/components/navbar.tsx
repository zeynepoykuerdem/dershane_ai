  ...
  {showNotif && (
    <div className="absolute right-0 top-10 w-80 bg-white border border-gray-200 rounded-xl shadow-lg z-50">
      <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100">
        <p className="text-sm font-semibold text-gray-800">Bildirimler</p>
      </div>
      {notifications.length === 0 ? (
        ...
        <div className="max-h-72 overflow-y-auto">
          {notifications.map((n) => (
            <div key={n.id} className="flex items-start justify-between gap-3 px-4 py-3 border-b border-gray-50 hover:bg-gray-50">
              ...
              <button onClick={() => markAsRead(n.id)} className="text-xs text-gray-300 hover:text-gray-500">✕</button>
            </div>
          ))}
        </div>
      )}
