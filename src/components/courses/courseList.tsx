...
export function CourseList(){
    ...
    const handleChangePage = (direction: 'prev' | 'next') => {
        setCurrentWeek(prev => {
            ...
            const newDate = new Date(prev.getFullYear(), prev.getMonth(), prev.getDate() + diff)
    ...
    const coursesByDay = DAYS.reduce((acc, day) => {
        acc[day] = courses.filter(c => getDayName(c.course_date) === day)
          .sort((a, b) => a.start_hour?.localeCompare(b.start_hour))
        return acc
      }, {} as Record<string, any[]>)

      if (loading) {
        return (<div className="flex items-center justify-center py-16 text-sm text-gray-400">
            <p className="text-gray-400 text-sm"> Yükleniyor...</p></div>)
      }
      if ( courses.length ===0) {
        return (<div className="text-center py-16 border border-dashed border-gray-200 rounded-xl">
