...
export function HomeworkList() {
  ...
  const handleComplete = async (id: number, current: boolean) => {
    const next = !current;
    setHomeworks((prev) =>
      prev.map((hw) => (hw.id === id ? { ...hw, is_completed: next } : hw)),
    );
    ...
    if (error) {
      ...
      setHomeworks((prev) =>
        prev.map((hw) =>
          hw.id === id ? { ...hw, is_completed: current } : hw,
        ),
      );
    }
  };
  ...
  const groups = {
    yapilacak: homeworks
      .filter(
        (hw) => !hw.is_completed && !isOverdue(hw.due_date, hw.is_completed),
      )
      .sort(byDate),
    tamamlandi: homeworks.filter((hw) => hw.is_completed).sort(byDate),
    gecikis: homeworks
      .filter((hw) => isOverdue(hw.due_date, hw.is_completed))
      .sort(byDate),
  };
